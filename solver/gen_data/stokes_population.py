"""Population sampling for the paper-corpus fifth-order Stokes family.

The population has four allocation cells: finite- and deep-water branches,
...
from solver.data.stokes_truth_jax import (
...
STOKES_POPULATION_CELLS = (
    StokesPopulationCell("finite_low", "finite", 0),
    StokesPopulationCell("finite_moderate", "finite", 1),
    StokesPopulationCell("deep_low", "deep", 0),
    StokesPopulationCell("deep_moderate", "deep", 1),
)
_CELL_BY_ID = {cell.cell_id: cell for cell in STOKES_POPULATION_CELLS}
    ...
    def to_json_record(self) -> JsonRecord:
        ...
        record = {
            **_assignment_record(self.assignment),
            "schema": "stokes_population_spec_v1",
            "status": "accepted",
...
class StokesPopulationSamplingError(ValueError):
...
def population_cell(cell_id: str) -> StokesPopulationCell:
    """Return the declared Stokes population cell named by ``cell_id``."""

    try:
        return _CELL_BY_ID[cell_id]
    except KeyError as error:
        raise ValueError(f"unknown Stokes population cell: {cell_id}") from error
...
def stokes_support_violations(
    ...
    """Return every violation of the declared Stokes population support."""
...
def sample_stokes_population(
    ...
    *,
    ...
    """Sample one complete paper-corpus Stokes specification.
    ...
    raise StokesPopulationSamplingError(

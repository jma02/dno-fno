"""Transactional sampler-to-trajectory adapters for the paper corpus.
...
def resolved_band_for_contract(
    ...
    *,
    ...
    """Return the random-sea band with transition at ``3 K / 4``."""
...
def sample_tanaka_trajectory_cases(
    ...
    *,
    contract: ResidualControlledGL2Contract,
) -> SampledTrajectoryCases[TanakaPopulationSample]:
...
def construct_tanaka_trajectory_batch(
    proposed: PersistedTrajectoryProposal[TanakaPopulationSample],
    *,
    ...
    if not all(isinstance(sample, TanakaPopulationSample) for sample in samples):
        raise TypeError("Tanaka construction requires Tanaka population samples")
...
def sample_benjamin_feir_trajectory_cases(
    ...
    *,
    contract: ResidualControlledGL2Contract,
) -> SampledTrajectoryCases[BenjaminFeirPopulationSample]:
    ...
    records = tuple(
        _strict_record(
            sample.to_json_record(),
            constructor="jcp09_equation_33_with_project_fifth_order_carrier_v1",
            contract=contract,
        )
        for sample in samples
    )
...
def construct_benjamin_feir_trajectory_batch(
    proposed: PersistedTrajectoryProposal[BenjaminFeirPopulationSample],
) -> TrajectoryInitialBatch:
    """Construct Benjamin--Feir states after their durable proposal."""

    sampled = proposed.sampled
    samples = sampled.samples
    if not all(isinstance(sample, BenjaminFeirPopulationSample) for sample in samples):
...
def sample_jonswap_tma_trajectory_cases(
    ...
    *,
    contract: ResidualControlledGL2Contract,
    quadrature_order: int = 16,
) -> SampledTrajectoryCases[JonswapTmaPopulationSample]:
    ...
    samples = tuple(
        sample_jonswap_tma_population(assignment, band=band) for assignment in attempted
    )
    settings: SpecificationRecord = {
...
def construct_jonswap_tma_trajectory_batch(
    proposed: PersistedTrajectoryProposal[JonswapTmaPopulationSample],
) -> TrajectoryInitialBatch:
    """Construct resolved-band JONSWAP/TMA states after durable proposal."""

    sampled = proposed.sampled
    samples = sampled.samples
    if not all(isinstance(sample, JonswapTmaPopulationSample) for sample in samples):
    ...
    eta0 = np.stack(tuple(state.eta for state in states))
    xi0 = np.stack(tuple(state.xi for state in states))
    depths = np.asarray(
        [sample.parameters.depth for sample in samples],
        dtype=np.float64,
    )
    return _batch(
        eta0,

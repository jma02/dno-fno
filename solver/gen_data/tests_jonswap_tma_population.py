"""CPU tests for the paper-corpus JONSWAP/TMA population sampler."""
...
class JonswapTmaPopulationTest(unittest.TestCase):
    def test_cells_are_exact_cartesian_product(self) -> None:
        ...
        self.assertEqual(len(JONSWAP_TMA_POPULATION_CELLS), 27)
        self.assertEqual(coordinates, expected)
        self.assertEqual(
            len({cell.cell_id for cell in JONSWAP_TMA_POPULATION_CELLS}),
            27,
        )
    ...
    def test_replay_is_bitwise_deterministic(self) -> None:
        first = sample_jonswap_tma_population(assignment(13), band=BAND)
        second = sample_jonswap_tma_population(assignment(13), band=BAND)
    ...
    def test_phases_are_explicit_independent_and_json_ready(self) -> None:
        sample = sample_jonswap_tma_population(assignment(8), band=BAND)
    ...
    def test_shallow_draws_obey_parameterization_and_constraint(self) -> None:
        ...
        for attempt_index in range(256):
            ...
            self.assertLessEqual(depth_wavenumber * relative_height, 0.15)

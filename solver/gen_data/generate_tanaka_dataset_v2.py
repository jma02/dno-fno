    ...
    *,
    ...
    if bool(np.all(finite)):
        ...
        if math.isfinite(speed_squared) and speed_squared > 0.0:
            minimum_scaled_radicand = _finite_or_none(minimum_radicand / speed_squared)
...
def _validate_tanaka_surface_potential_radicand(
    ...
    *,
    ...
    # This is the only device-to-host transfer of the full radicand.  Domain
    # validation deliberately happens before taking the square root.
...
def build_per_case_initial_conditions(
    *,
    ...
    # Reconstruct each smooth parametric profile using its native tangent
    # d(eta)/dx = tan(theta). Piecewise-linear placement leaves derivative
    # corners whose small Fourier tail is amplified by the order-6 DNO series.
    # Restriction from the existing 8N grid is retained so no other part of the
    ...
    eta_per_crest = jax.vmap(
        lambda x_row, eta_row, theta_row, h, center: place_tanaka_profile_periodic(
            x_row,
            eta_row,
            theta_row,
            h,
    ...
    xi_x = signed_speed - direction_col * jnp.sqrt(radical)

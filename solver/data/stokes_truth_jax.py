...
def stokes_eta_xi(
    ...
    if ichoi == 0:
        ...
        om = om0 * (1.0 + 0.5 * eps2 + 5.0 * eps4 / 8.0)
        theta = k0 * x - om * time
        eta = (
            (1.0 + eps2 / 8.0 + 121.0 * eps4 / 192.0) * jnp.cos(theta)
            + (0.5 * eps + 5.0 * eps3 / 6.0) * jnp.cos(2.0 * theta)
            + (3.0 * eps2 / 8.0 + 171.0 * eps4 / 128.0) * jnp.cos(3.0 * theta)
            + eps3 * jnp.cos(4.0 * theta) / 3.0
            + 125.0 * eps4 * jnp.cos(5.0 * theta) / 384.0
        )
        eta = a0 * eta
        xi = jnp.exp(k0 * eta) * jnp.sin(theta)
        xi = xi + 0.5 * eps3 * jnp.exp(2.0 * k0 * eta) * jnp.sin(2.0 * theta)
        xi = xi + eps4 * jnp.exp(3.0 * k0 * eta) * jnp.sin(3.0 * theta) / 12.0
        xi = a0 * om0 * xi / k0
        return eta, xi
    ...
    eta = (
        (1.0 + coeffs["eps2"] * coeffs["B31"] + coeffs["eps4"] * coeffs["B51"]) * jnp.cos(theta)
        + coeffs["eps"] * (coeffs["B22"] + coeffs["eps2"] * coeffs["B42"]) * jnp.cos(2.0 * theta)
        + coeffs["eps2"] * (coeffs["B33"] + coeffs["eps2"] * coeffs["B53"]) * jnp.cos(3.0 * theta)
        + coeffs["eps3"] * coeffs["B44"] * jnp.cos(4.0 * theta)
        + coeffs["eps4"] * coeffs["B55"] * jnp.cos(5.0 * theta)
    )
    eta = a0 * eta
    ...
    xi = (
        ...
        * jnp.sin(3.0 * theta)
        + coeffs["eps3"] * coeffs["A44"] * jnp.cosh(4.0 * k0 * (eta + depth)) * jnp.sin(4.0 * theta)
        + coeffs["eps4"] * coeffs["A55"] * jnp.cosh(5.0 * k0 * (eta + depth)) * jnp.sin(5.0 * theta)
        + (coeffs["eps"] * coeffs["C2"] + coeffs["eps3"] * coeffs["C4"]) * coeffs["om0"] * time / coeffs["sigma"]
    )

#!/usr/bin/env python3
"""Watch the exact long-running paper-corpus workers for durable progress.
...
DEFAULT_JONSWAP_ROOT: Final = (
    ROOT / "outputs/paper_corpus_jonswap_revision4_relative_band_v1"
)
DEFAULT_TANAKA_ROOT: Final = (
    ROOT / "outputs/paper_corpus_revision3_literature_aligned_v1"
)
DEFAULT_FINAL_VIEW_SUMMARY: Final = (
    ROOT
    / "outputs/paper_corpus_literature_aligned_v1"
    / "combined/c16384_v01024_t01024"
    / "paper_corpus_all_splits_c16384.summary.json"
)
DEFAULT_LOG: Final = ROOT / "outputs/paper_corpus_worker_watchdog.log"

Family = Literal["jonswap_tma", "tanaka"]
Phase = Literal["running", "term_sent", "kill_sent"]

FINAL_VIEW_RELATIVE_PATH: Final = Path(
    "outputs/paper_corpus_literature_aligned_v1/combined/"
    "c16384_v01024_t01024/paper_corpus_all_splits_c16384.summary.json"
)
FINAL_VIEW_NAME: Final = "paper_corpus_all_splits_c16384"
...
FINAL_SOURCE_ROOTS: Final = {
    "stokes": Path("outputs/paper_corpus_cap4_revision2_20260728"),
    "tanaka": Path("outputs/paper_corpus_revision3_literature_aligned_v1"),
    "benjamin_feir": Path(
        "outputs/paper_corpus_bf_revision4_jonswap_revision3_literature_aligned_v1"
    ),
    "jonswap_tma": Path("outputs/paper_corpus_jonswap_revision4_relative_band_v1"),
}
...
FINAL_BENJAMIN_FEIR_POPULATION_SUPPORT_FINGERPRINT: Final = (
    "5e1e9dca28877356dc1b8e6a6538ee69a20f0825d9dc11f7bb1f61667656ed6d"
)
...
def _validate_source_summary(
    ...
    *,
    ...
    if (
        record.get("schema") != "paper_corpus_quota_summary_v1"
    ...
    if (
        run_spec.get("schema") != "paper_corpus_accepted_quota_run_v2"
    ...
    if plan.family == "stokes":
        ...
        configuration_keys.add("population_support")
    ...
    if (
        configuration.get("schema") != "paper_corpus_quota_configuration_v1"
    ...
    if plan.family == "stokes":
        ...
        if (
            _canonical_json_sha256(configuration.get("population_support"))
            != FINAL_BENJAMIN_FEIR_POPULATION_SUPPORT_FINGERPRINT
        ):
            raise ValueError("Benjamin--Feir population support differs")
    elif plan.family == "jonswap_tma":
    ...
    source_name = f"paper_corpus_{plan.family}_{plan.split}"
    ...
    if (
        preflight.get("schema") != "paper_corpus_quota_preflight_v1"
...
def _authenticate_final_view(path: Path, *, repository_root: Path) -> None:
    ...
    if (
        summary.get("schema") != "paper_corpus_combined_view_summary_v1"
    ...
    if (
        preflight.get("schema") != "paper_corpus_combined_view_preflight_v1"
        or preflight.get("mode") != "dry_run"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        # source-frozen corpus, for which no completed transaction exists yet.
        default=6.0 * 60.0 * 60.0,
    )
    parser.add_argument(
        "--kill-grace-seconds",

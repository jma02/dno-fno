...
from scripts import build_paper_dataset_view as view_builder
...
from scripts import build_parameterized_dataset_case_figure as figure_producer
from scripts import render_paper_dataset_worst_cases as renderer_producer
    ...
    def test_module_help_runs_from_repository_root(self) -> None:
        ...
        completed = subprocess.run(
            [
                sys.executable,
                "-m",
                "scripts.build_paper_dataset_document_handoff",
                "--help",
            ],
            cwd=repository_root,
            check=False,
...
class DocumentHandoffTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.dataset_contract = {
            "schema": "synthetic_paper_dataset_contract_v1",
    ...
    def _semantic_plan(
        ...
        *,
        namespace: str,
        case_id_base: int,
    ) -> view_builder.CombinedDatasetPlan:
        ...
        return view_builder.CombinedDatasetPlan(
            chunks=tuple(chunks),
    ...
    def _expected_preflight(self, training_cases: int) -> dict[str, object]:
        ...
        return {
            ...
            "view_name": f"paper_dataset_all_splits_c{tag}",
    ...
    def _write_semantic_view(
        ...
        *,
        root: Path,
        name: str,
        plan: view_builder.CombinedDatasetPlan,
    ...
    def _write_combined_view(
        ...
        tag = f"{training_cases:05d}"
        name = f"paper_dataset_all_splits_c{tag}"
    ...
    def _chunks(self) -> tuple[dict[str, object], ...]:
        ...
        for family in runner.FAMILY_ORDER:
            for split, before, count, stream in runner.expected_layout(family):
                ...
                source_root.mkdir()
                stem = f"paper_dataset_{family}_{split}"
    ...
    def _training(self) -> None:
        ...
        full_row_selection = {
            ...
            "splits": {
                split: {
                    "count": sum(
                        runner.CASES_PER_FAMILY_BY_SPLIT[split]
                        * runner.ROWS_PER_CASE[family]
                        for family in runner.FAMILY_ORDER
                    ),
                    "sha256": digest * 64,
                    ...
                    "per_family_row_count": {
                        family: runner.CASES_PER_FAMILY_BY_SPLIT[split]
                        * runner.ROWS_PER_CASE[family]
                        for family in runner.FAMILY_ORDER
                    },
                }
                for split, digest in zip(runner.SPLIT_ORDER, "abc")
            },
        }
        _write_json(

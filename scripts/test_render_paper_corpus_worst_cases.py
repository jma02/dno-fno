"""Focused tests for final-corpus binding and morphology diagnostics."""
...
from scripts import render_paper_corpus_worst_cases as renderer  # noqa: E402

from scripts.render_paper_corpus_worst_cases import (  # noqa: E402
    COMBINED_PREFLIGHT_SCHEMA,
    COMBINED_SUMMARY_SCHEMA,
    FINAL_PAPER_CORPUS_ACCEPTED_CASES,
    FINAL_PAPER_CORPUS_RETAINED_ROWS,
    FINAL_PAPER_CORPUS_SOURCE_COUNT,
...
def _combined_fixture(root: Path) -> tuple[Path, tuple[Path, ...]]:
    ...
    for index, family in enumerate(("tanaka", "benjamin_feir")):
        ...
        source_root.mkdir()
        summary_path = source_root / f"paper_corpus_{family}_train.summary.json"
    ...
    combined_path = root / "paper_corpus_all_splits_c16384.summary.json"
    _write_json(
        combined_path,
        {
...
def _source_fixture(
    ...
    *,
    ...
    stem = f"paper_corpus_{family}_train"
...
class CombinedSummaryBindingTests(unittest.TestCase):
    ...
    def test_source_and_combined_summary_options_are_mutually_exclusive(self) -> None:
        argv = [
            "render_paper_corpus_worst_cases.py",
            "--source",
            "chunk",
            "--combined-summary",
            "combined.json",
...
class FinalContractAndPublicationTests(unittest.TestCase):
    ...
    def _final_family_split_counts() -> dict[tuple[str, str], int]:
        return {
            ...
            for split, count in FINAL_PAPER_CORPUS_SPLIT_ACCEPTED_CASES.items()
        }

    def test_final_contract_accepts_only_exact_counts(self) -> None:
        expected = self._final_family_split_counts()
        validate_final_paper_corpus_counts(
            source_count=FINAL_PAPER_CORPUS_SOURCE_COUNT,
            family_split_accepted_cases=expected,
            accepted_cases=FINAL_PAPER_CORPUS_ACCEPTED_CASES,
            retained_rows=FINAL_PAPER_CORPUS_RETAINED_ROWS,
        )
        ...
        mismatches = (
            {
                "source_count": FINAL_PAPER_CORPUS_SOURCE_COUNT - 1,
                "family_split_accepted_cases": expected,
                "accepted_cases": FINAL_PAPER_CORPUS_ACCEPTED_CASES,
                "retained_rows": FINAL_PAPER_CORPUS_RETAINED_ROWS,
            },
            {
                ...
                "accepted_cases": FINAL_PAPER_CORPUS_ACCEPTED_CASES - 1,
                "retained_rows": FINAL_PAPER_CORPUS_RETAINED_ROWS,
            },
            {
                "source_count": FINAL_PAPER_CORPUS_SOURCE_COUNT,
                "family_split_accepted_cases": expected,
                "accepted_cases": FINAL_PAPER_CORPUS_ACCEPTED_CASES,
                "retained_rows": FINAL_PAPER_CORPUS_RETAINED_ROWS - 1,
            },
        )
        for values in mismatches:
            with (
                ...
                self.assertRaisesRegex(
                    ValueError, "final paper-corpus contract failed"
                ),
            ):
                validate_final_paper_corpus_counts(**values)

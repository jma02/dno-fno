"""Build one immutable, document-only handoff from completed corpus evidence.
...
does not discover or accept independent corpus paths.  It reauthenticates the
...
from scripts import run_paper_corpus_postcompletion as postcompletion


SCHEMA = "paper_corpus_document_handoff_v1"
QUOTA_SUMMARY_SCHEMA = "paper_corpus_quota_summary_v1"
...
FAMILY_AUDIT_SCHEMAS = {
    "stokes": "paper_corpus_stokes_revision2_completion_binding_v1",
    "tanaka": "paper_corpus_tanaka_revision3_completion_audit_v1",
    "benjamin_feir": "paper_corpus_benjamin_feir_revision4_completion_audit_v1",
    "jonswap_tma": "paper_corpus_jonswap_tma_revision4_completion_audit_v1",
}
STOKES_LEGACY_AUDIT_SCHEMA = "paper_corpus_cap4_stokes_audit_v1"
...
RENDERER_PARAMETER_FIELDS = (
    "final_paper_corpus_contract_required",
...
def _cumulative_view_path(final_summary: Path, training_cases: int) -> Path:
    ...
    return (
        ...
        / f"paper_corpus_all_splits_c{tag}.summary.json"
    )
...
def _authenticate_cumulative_view(
    ...
    *,
    ...
    tag = f"{training_cases:05d}"
    expected_name = f"paper_corpus_all_splits_c{tag}"
...
def _validate_completed_status(
    ...
    *,
    ...
    if (
        ...
        or status.get("diagnostics_affect_corpus_release") is not False
...
def _renderer_facts(authenticated: AuthenticatedPostcompletion) -> dict[str, object]:
    ...
    if (
        parameters.get("final_paper_corpus_contract_required") is not True
...
def _training_facts(authenticated: AuthenticatedPostcompletion) -> dict[str, object]:
    ...
    train_sha = _digest(train.get("sha256"), context="train sampler SHA")
    validation_sha = _digest(validation.get("sha256"), context="validation sampler SHA")
    stats_fingerprint = _digest(
        normalization.get("stats_fingerprint"),
        context="normalization statistics fingerprint",
    )
    if sampler.get("schema") != "paper_corpus_schema_v2_sampler_contract_v1":
        raise RuntimeError("training sampler has the wrong schema")
    extrema_names = (
        "feature_min",
        "feature_max",
        "target_min",
    ...
    return {
        ...
        "sampler": {
            "schema": sampler.get("schema"),
            "seed": sampler.get("seed"),
            "train_epoch_0": {
                "count": train.get("count"),
                "sha256": train_sha,
                "per_family_count": train.get("per_family_count"),
            },
            "fixed_validation_epoch_0": {
                "count": validation.get("count"),
                "sha256": validation_sha,
                "per_family_count": validation.get("per_family_count"),
            },
            "test_rows_in_train_or_validation": sampler.get(
...
def build_record(authenticated: AuthenticatedPostcompletion) -> dict[str, object]:
    ...
    record = {
        "schema": SCHEMA,
        "status": "complete",
        "purpose": "paper_corpus_document_update_handoff",

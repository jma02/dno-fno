...
from scripts import build_parameterized_corpus_case_figure as figure_builder
from scripts.build_parameterized_corpus_case_figure import (
...
from scripts.render_paper_corpus_worst_cases import (
...
def _source_fixture(
    ...
    *,
    ...
    map_path = root / f"paper_corpus_{family}_validation.trajectory_map.npz"
    np.savez(
        map_path,
    ...
    manifest_path = root / f"paper_corpus_{family}_validation.dataset.json"
    _write_json(
        manifest_path,
        {
    ...
    summary_path = root / f"paper_corpus_{family}_validation.summary.json"
    _write_json(
        summary_path,
        {
...
class ContractAndBindingTests(unittest.TestCase):
    ...
    def test_empty_population_cannot_satisfy_final_contract(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "final paper-corpus contract failed"):

...
os.environ.setdefault(
    "MPLCONFIGDIR",
    "/tmp/mpl-paper-corpus-training-handoff-tests",
)

import numpy as np

from scripts import audit_paper_corpus_training_handoff as handoff_audit
...
from scripts.audit_paper_corpus_training_handoff import (
    AUDIT_SCHEMA,
    ...
    hierarchical_case_time_selection,
    parse_args,
    write_audit,
)
...
SYNTHETIC_CONTRACT = CorpusContract(
...
def _fixture(root: Path) -> tuple[Path, Path, tuple[Path, ...]]:
    ...
    for split in SPLITS:
        for family in FAMILIES:
            ...
            source_view = build_dataset_view(
                source_root,
                (batch,),
                name=f"paper_corpus_{family}_{split}",
                expected_fingerprint=fingerprint,
            )
            source_summary = source_root / f"paper_corpus_{family}_{split}.summary.json"
            _write_json(
                source_summary,
                {
    ...
    combined_view = build_dataset_view(
        combined_root,
        tuple(batches),
        name="paper_corpus_all_splits_c00001",
    ...
    combined_summary = combined_root / "paper_corpus_all_splits_c00001.summary.json"
    _write_json(
        combined_summary,
        {
...
class StreamingTrainingHandoffAuditTests(unittest.TestCase):
    ...
    def test_streaming_stats_and_selection_match_trainer_exactly(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("--data_fraction", handoff["canonical_command_template"])

            _, validation_indices, _ = build_dataset_split_indices(dataset, seed=0)
            expected_epoch_train = build_hierarchical_epoch_indices(
                dataset,
                split_id=0,
                seed=0,
                epoch=0,
            )
            expected_validation = build_case_balanced_validation_indices(
                dataset,
                validation_indices,
                seed=0,
            )
            observed_sampler = audit["training_normalization"]["schema_v2_sampler"]
            self.assertEqual(
                observed_sampler["train_epoch_0"]["sha256"],
                _index_selection(expected_epoch_train)["sha256"],
            )
            self.assertEqual(
                observed_sampler["fixed_validation_epoch_0"]["sha256"],
                _index_selection(expected_validation)["sha256"],
            )
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            map_path = manifest_path.parent / manifest["trajectory_map_npz"]
            ...
            self.assertEqual(
                ...
                expected_epoch_train.tolist(),
            )
            self.assertEqual(observed_sampler["test_rows_in_train_or_validation"], 0)

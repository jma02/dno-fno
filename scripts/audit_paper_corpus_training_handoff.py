"""Audit the final paper-corpus view and stream its training normalization.
...
extrema without concatenating the corpus fields in memory.
"""
...
os.environ.setdefault(
    "MPLCONFIGDIR",
    "/tmp/mpl-paper-corpus-training-handoff",
)
...
from solver.gen_data.pipeline.production import (
    PAPER_CORPUS_REVISION_BY_FAMILY,
    PhysicalFamilyId,
    SplitId,
    split_code,
)
...
AUDIT_SCHEMA = "paper_corpus_training_handoff_audit_v1"
...
SOURCE_SUMMARY_SCHEMA = "paper_corpus_quota_summary_v1"
...
FINAL_CONTRACT = CorpusContract(
    ...
    revision_by_family={
        family: int(PAPER_CORPUS_REVISION_BY_FAMILY[PhysicalFamilyId(family_id)])
...
def load_and_authenticate(
    ...
    *,
    contract: CorpusContract = FINAL_CONTRACT,
    authenticator: FileAuthenticator | None = None,
...
def training_row_selection(
    ...
    candidates = np.flatnonzero(accepted & (row_split == SPLIT_IDS["train"]))
    indices = np.random.default_rng(seed).permutation(candidates)
    return indices, _selection(indices)


def hierarchical_case_time_selection(
    arrays: Mapping[str, np.ndarray],
    *,
    split: str,
    seed: int,
    ...
    """Reproduce the schema-v2 one-time-per-case sampler from map metadata."""
    ...
    accepted = arrays["trajectory_accepted"]
    split_ids = arrays["trajectory_split_id"]
    families = arrays["trajectory_family_id"]
    first_rows = arrays["trajectory_first_row"]
    row_counts = arrays["trajectory_row_count"]
    trajectories = np.flatnonzero(accepted & (split_ids == SPLIT_IDS[split]))
    if trajectories.size == 0:
        return np.empty((0,), dtype=np.int64)
    trajectories_by_family = {
        family: trajectories[families[trajectories] == family].tolist()
        for family in sorted(int(value) for value in np.unique(families[trajectories]))
    }
    family_case_counts = {
    ...
    if len(set(family_case_counts.values())) != 1:
        raise RuntimeError(
            "hierarchical sampling requires equal accepted case counts per family"
        )
    rng = np.random.default_rng(np.random.SeedSequence([seed, epoch]))
    selected_rows: list[int] = []
    for family in sorted(trajectories_by_family):
        family_trajectories = rng.permutation(trajectories_by_family[family])
        selected_rows.extend(
            int(first_rows[int(trajectory)])
            + int(rng.integers(int(row_counts[int(trajectory)])))
            for trajectory in family_trajectories
        )
    return np.asarray(rng.permutation(selected_rows), dtype=np.int64)


def sampler_contract_record(
    arrays: Mapping[str, np.ndarray],
    *,
    contract: CorpusContract,
    seed: int,
) -> dict[str, object]:
    """Audit the epoch-0 train/validation selections and test exclusion."""

    train = hierarchical_case_time_selection(arrays, split="train", seed=seed, epoch=0)
    validation = hierarchical_case_time_selection(
        arrays, split="validation", seed=seed, epoch=0
    )
    owners = arrays["trajectory_index"]
    trajectory_families = arrays["trajectory_family_id"]
    trajectory_splits = arrays["trajectory_split_id"]
    expected_counts = {
        "train": len(FAMILY_IDS) * contract.accepted_cases_per_family_by_split["train"],
        "validation": len(FAMILY_IDS)
        * contract.accepted_cases_per_family_by_split["validation"],
    }
    selections = {"train": train, "validation": validation}
    ...
    for split, selected in selections.items():
        ...
        if (
            ...
            or np.unique(selected_owners).size != selected.size
            or np.any(trajectory_splits[selected_owners] != SPLIT_IDS[split])
        ):
            raise RuntimeError(
                f"{split} epoch-0 selection violates one-row-per-case sampling"
            )
        per_family = {
            family: int(
        ...
        expected_per_family = contract.accepted_cases_per_family_by_split[split]
        if set(per_family.values()) != {expected_per_family}:
            raise RuntimeError(f"{split} epoch-0 family counts differ from contract")
        selection_records[split] = {
            "epoch": 0,
            "policy": "one_uniformly_selected_stored_time_per_accepted_case",
    ...
    if test_leakage != 0:
        raise RuntimeError("epoch-0 train/validation selections leak test cases")
    return {
        "schema": "paper_corpus_schema_v2_sampler_contract_v1",
        "seed": seed,
        "train_epoch_0": selection_records["train"],
        "fixed_validation_epoch_0": selection_records["validation"],
...
def audit_training_handoff(
    ...
    *,
    ...
    sampler_contract = sampler_contract_record(
        loaded.map_arrays,
        contract=contract,
        seed=seed,
    )
    ...
    record: dict[str, object] = {
        ...
        "training_normalization": {
            ...
            "schema_v2_sampler": sampler_contract,
            "stats": stats,

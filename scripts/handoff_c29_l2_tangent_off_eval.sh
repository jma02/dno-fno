#!/bin/bash
# Evaluate C29 automatically after its exact tangent-off retrain.

set -euo pipefail
cd /home/johnma/dno-fno
export PATH="/home/johnma/.local/bin:/usr/local/bin:/usr/bin:/bin"

: "${RUN_NAME:?RUN_NAME must identify the C29 training run}"
...
ACCEPTANCE_OUTPUT="$RUN_DIR/c27_final_n32_tangent_ablation_acceptance.json"

exec 9>"$LOCK_FILE"
if ! flock -n 9; then
  echo "$(date '+%Y-%m-%d %H:%M:%S %Z') C29 evaluation handoff already active"
  exit 0
fi

if [[ -f "$ACCEPTANCE_OUTPUT" ]]; then
  echo "$(date '+%Y-%m-%d %H:%M:%S %Z') C29 evaluation already complete"
  exit 0
fi
...
launcher_active() {
  pgrep -f "[b]ash scripts/launch_c29_l2_tangent_off_full.sh" >/dev/null
}

if [[ ! -d "$RUN_DIR" ]] && ! training_active && ! launcher_active; then
  echo "C29 has not been launched: $RUN_DIR" >&2
  exit 1
fi
...
if differences != {"translation_tangent_weight": (10.0, 0.0)}:
    raise SystemExit(f"unexpected C27/C29 config differences: {differences}")
...
for record in records:
    ...
    if record.get("translation_tangent_extra") not in (None, 0.0):
        raise SystemExit(
            f"epoch {record['epoch']}: tangent loss contributed to objective"
        )
...
print("C29 training guard passed: exact finite epoch-40 tangent-only deletion")
PY
...
C29_NON_TANAKA="$(<"$RUN_DIR/last_eval_guarded_non_tanaka_suite_n32.txt")"
...
uv run python scripts/compare_c27_c28_mode_ablation.py \
  ...
  --candidate-tanaka-dir "$C29_TANAKA" \
  --candidate-non-tanaka-dir "$C29_NON_TANAKA" \
  --candidate-run-dir "$RUN_DIR" \
  --comparison "C29 tangent-off epoch-40 final versus C27 epoch-40 final" \
  --decision-label "C29 tangent-loss ablation" \
  --output "$ACCEPTANCE_OUTPUT"

printf '%s\n' "$C29_TANAKA" >"$RUN_DIR/last_final_tanaka64_eval.txt"
printf '%s\n' "$C29_NON_TANAKA" >"$RUN_DIR/last_final_non_tanaka_n32_eval.txt"
echo "$(date '+%Y-%m-%d %H:%M:%S %Z') C29 final evaluation complete"

...
from scripts import run_paper_dataset_postcompletion as runner
...
class PostcompletionRunnerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.inputs.mkdir()
        self.combined = self.inputs / "paper_dataset_all_splits_c16384.summary.json"
        _write_json(
            self.combined,
...
""",
    ...
    def _strict_stage_fixture(self):
        from scripts.test_build_paper_dataset_document_handoff import (
        ...
        return runner.ReleaseBinding(
            ...
            historical_source_snapshots={
                "stokes": {
                    ...
                    "scripts/run_paper_dataset_quota.py": runner._artifact(
                        self.release_stokes_snapshot_root / "run_paper_dataset_quota.py"
                    ),
    ...
    def test_terminal_document_handoff_prevents_any_rerun_mutation(self) -> None:
        from scripts import build_paper_dataset_document_handoff as handoff
    ...
    def test_stokes_audit_requires_exact_source_closure_checks(self) -> None:
        for check in (
            ...
            "exact_equal_13_path_source_maps",
    ...
    @unittest.skipUnless(
        (
            runner.ROOT / "outputs/paper_dataset_cap4_revision2_20260728/"
    ...
    def test_current_real_stokes_and_bf_artifacts_authenticate_read_only(self) -> None:
        stokes_root = runner.ROOT / "outputs/paper_dataset_cap4_revision2_20260728"
...
class CombinedSummaryAuthenticationTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory()
        self.root = Path(self.temporary.name)
        self.name = "paper_dataset_all_splits_c16384"
...
class ReleaseGenerationIdentityTests(unittest.TestCase):
    ...
    def test_current_tanaka_without_historical_compatibility_reconstructs(self) -> None:
        ...
        plan = runner.combined_view_builder.CombinedDatasetPlan(
            chunks=(chunk,),
    ...
    def test_all_four_exact_maps_bind_30_unique_current_files(self) -> None:
        ...
        self.assertEqual(
            ...
            {"stokes": 13, "tanaka": 20, "benjamin_feir": 19, "jonswap_tma": 20},
        )
        self.assertEqual(
            {
            ...
            {"stokes": 10, "tanaka": 20, "benjamin_feir": 17, "jonswap_tma": 20},
        )
        self.assertEqual(
            {
    ...
    def test_current_source_direct_and_ancestor_symlinks_fail_closed(self) -> None:
        direct = self.repository / "solver/gen_data/tanaka_initial_conditions.py"
    ...
    def test_terminal_sweep_catches_early_source_mutated_during_later_hash(
        ...
        combined, audits, sources = self._global_material()
        early = self.repository / "scripts/run_paper_dataset_quota.py"

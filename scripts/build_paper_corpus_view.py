...
def load_completed_chunk(
    ...
    *,
    ...
    current_revision = paper_corpus_revision_id(FAMILY_IDS[spec.family_name])
    if spec.revision_id != current_revision:
        raise ValueError(
            f"{spec.family_name} chunk revision {spec.revision_id} is not "
            f"current paper-corpus revision {current_revision}"
        )
    execution_key = (
...
def validate_combined_plan(
    ...
    for chunk in values:
        current_revision = paper_corpus_revision_id(FAMILY_IDS[chunk.family])

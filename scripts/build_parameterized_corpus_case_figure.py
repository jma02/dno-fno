"""Build deterministic family illustrations from the completed paper corpus.
...
owned stored row.  The completed combined-view summary is the sole corpus
...
from scripts.render_paper_corpus_worst_cases import (  # noqa: E402
...
SIDECAR_SCHEMA = "paper_corpus_deterministic_family_illustrations_v1"
IMPLEMENTATION_SCHEMA = "paper_corpus_family_figure_implementation_v1"
IMPLEMENTATION_FILES: Final = (
    (
        "scripts/build_parameterized_corpus_case_figure.py",
...
EXPECTED_CELL_IDS: Final = {
    ...
    "benjamin_feir": tuple(cell.cell_id for cell in BENJAMIN_FEIR_POPULATION_CELLS),
    "jonswap_tma": tuple(cell.cell_id for cell in JONSWAP_TMA_POPULATION_CELLS),
}
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-stem",
        type=Path,
        default=ROOT / "notes/figures/parameterized_corpus_case_examples",
        help=(
...
def _validate_final_plan_layout(plan: CombinedCorpusPlan) -> None:
...
def load_source_identity(source: CorpusSource) -> SourceIdentity:
...
def validate_release_population(
    ...
    if observed_revisions != expected_revisions:
        raise ValueError(
            "final paper-corpus revisions differ: "
...
def _render_figure(
    ...
    *,
    ...
    figure.savefig(
        ...
        bbox_inches="tight",
        metadata={
            "Creator": "build_parameterized_corpus_case_figure.py",
            "CreationDate": None,
            "ModDate": None,
        },
    )
    figure.savefig(
        ...
        dpi=220,
        bbox_inches="tight",
        metadata={"Software": "build_parameterized_corpus_case_figure.py"},
    )
    plt.close(figure)
...
def build_figure(
    ...
    *,
    ...
    """Validate the final corpus, select four cases, and publish the figure."""

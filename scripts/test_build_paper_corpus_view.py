"""Structural and source-binding tests for combined paper-corpus views."""
...
from scripts import run_paper_corpus_jonswap_bucketed as bucketed
...
def _chunk(
    ...
    *,
    ...
    selected_revision = (
        paper_corpus_revision_id(FAMILY_IDS[family])
...
def _trajectory_map_arrays(plan: CombinedCorpusPlan) -> dict[str, np.ndarray]:
    family_ids: list[int] = []
...
def _all_split_plan(root: Path) -> CombinedCorpusPlan:
...
class CombinedPaperCorpusViewTests(unittest.TestCase):
    ...
    def test_real_retained_tanaka_identity_resolves_only_via_bridge(self) -> None:
        summaries = tuple(
            Path("outputs/paper_corpus_revision3").glob(
                "**/paper_corpus_tanaka_*.summary.json"
            )
        )
        if not summaries:
    ...
    def test_current_tanaka_source_does_not_require_legacy_bridge(self) -> None:
        ...
        resolution = _resolve_generation_compatibility(
            ...
            revision_id=paper_corpus_revision_id(FAMILY_IDS["tanaka"]),
    ...
    def test_equal_nested_chunks_have_expected_case_and_row_counts(self) -> None:
        ...
        self.assertEqual(
            ...
            {
                family: paper_corpus_revision_id(FAMILY_IDS[family])
                for family in FAMILY_ORDER
            },
        )
        self.assertEqual(
    ...
    def test_paper_view_rejects_noncurrent_family_revision(self) -> None:
        for stale_family in FAMILY_ORDER:
            with self.subTest(family=stale_family):
                current_revision = paper_corpus_revision_id(FAMILY_IDS[stale_family])
                chunks = tuple(
                    _chunk(
                        family,
                        before=0,

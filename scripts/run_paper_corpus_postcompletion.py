"""Run final, read-only paper-corpus diagnostics after generation completes.
...
The family audits and combined corpus are release evidence.  This runner never
writes to them.  Its stages are diagnostics: a stage failure is recorded and
causes a nonzero exit, but does not alter corpus acceptance or release status.
"""
...
if __package__:
    from scripts import build_paper_corpus_view as combined_view_builder
else:  # Direct ``python scripts/<name>.py`` entrypoint.
    import build_paper_corpus_view as combined_view_builder
...
RUNNER_SCHEMA = "paper_corpus_postcompletion_runner_v1"
DOCUMENT_HANDOFF_NAME: Final = "paper_corpus_document_handoff.json"
RELEASE_IDENTITY_SCHEMA = "paper_corpus_postcompletion_release_identity_v2"
COMBINED_SUMMARY_SCHEMA = "paper_corpus_combined_view_summary_v1"
COMBINED_PREFLIGHT_SCHEMA = "paper_corpus_combined_view_preflight_v1"
DEPENDENCY_BINDING_SCHEMA = "paper_corpus_postcompletion_dependency_binding_v1"
GENERATION_SOURCE_BINDING_SCHEMA = (
    "paper_corpus_postcompletion_generation_source_binding_v1"
)
RENDERER_SCHEMA = "paper_corpus_all_family_diagnostic_tail_v3"
RENDERER_IMPLEMENTATION_SCHEMA = "paper_corpus_renderer_implementation_v1"
ORDER_SCHEMA = "paper_corpus_jonswap_revision4_order_convergence_v1"
ORDER_IMPLEMENTATION_SCHEMA = "paper_corpus_order_diagnostic_implementation_v1"
TRAINING_SCHEMA = "paper_corpus_training_handoff_audit_v1"
TRAINING_IMPLEMENTATION_SCHEMA = "paper_corpus_training_implementation_v1"
FIGURE_SCHEMA = "paper_corpus_deterministic_family_illustrations_v1"
FIGURE_IMPLEMENTATION_SCHEMA = "paper_corpus_family_figure_implementation_v1"

TRAINING_IMPLEMENTATION_FILES: Final = (
    (
        "scripts/audit_paper_corpus_training_handoff.py",
    ...
    (
        "scripts/test_audit_paper_corpus_training_handoff.py",
...
TRAINING_IMPLEMENTATION_RELATIONSHIP: Final = {
    ...
    "regression_test_path": "scripts/test_audit_paper_corpus_training_handoff.py",
    "claim_scope": (
...
TANAKA_EXPECTED_GENERATION_SOURCE_PATHS: Final = frozenset(
    {
        "scripts/run_paper_corpus_quota.py",
        "solver/gen_data/adaptive_sampling.py",
        ...
        "solver/gen_data/pipeline/trajectory_writer.py",
        "solver/gen_data/pipeline/writer.py",
        "solver/gen_data/tanaka_population.py",
        "solver/gen_data/trajectory_family_adapters.py",
        "solver/gen_data/trajectory_quota_executor.py",
        "solver/solvers/dno_series_jax.py",
        "solver/solvers/time_integrator.py",
    ...
    def identity_record(self) -> dict[str, object]:
        ...
        return {
            ...
            "historical_source_snapshots": {
                "schema": (
                    "paper_corpus_postcompletion_historical_source_snapshots_v2"
                ),
...
def _reconstruct_dataset_contract(
    plan: combined_view_builder.CombinedCorpusPlan,
) -> dict[str, object]:
...
def authenticate_combined_summary(path: Path) -> CombinedBinding:
    ...
    if summary_artifact.path.name != "paper_corpus_all_splits_c16384.summary.json":
    ...
    view_name = "paper_corpus_all_splits_c16384"
...
def _release_terminal_artifact_groups(
    *,
    ...
    for relative, artifact in dependency.files.items():
        ...
        current[relative] = artifact
    if len(current) != 32 + len(DEPENDENCY_FILE_PATHS):
        raise RuntimeError("release current-source/dependency sweep is incomplete")
...
def _authenticate_release_generation_identity(
    *,
    ...
    if set(generation_sources) != set(FAMILY_ORDER) or len(artifact_cache) != 32:
...
def _stokes_source_maps(
    ...
    """Parse one exact 14-path map independently from all six summaries."""
    ...
    if set(first) != STOKES_EXPECTED_GENERATION_SOURCE_PATHS:
        ...
        raise RuntimeError(
            "Stokes source summaries differ from the frozen 14-path contract: "
...
def _validate_stokes_source_identity(
    ...
    *,
    ...
    if (
        ...
        != "paper_corpus_stokes_revision2_historical_source_binding_v1"
    ...
    if (
        current.get("schema")
        != "paper_corpus_stokes_revision2_current_source_binding_v1"
        or current.get("role")
...
def _validate_bf_source_identity(
    ...
    *,
    ...
    if (
        historical.get("schema")
        != "paper_corpus_bf_revision4_historical_source_binding_v1"
        or historical.get("role") != "inert_historical_byte_recovery_only"
        or historical.get("snapshot_root") != str(resolved_snapshot_root)
        or historical.get("chunk_source_maps_checked")
    ...
    if (
        current.get("schema") != "paper_corpus_bf_revision4_current_source_binding_v1"
        or current.get("role")
        != "current_repository_bytes_for_all_nonhistorical_generation_sources"
        or current.get("repository_root") != str(resolved_repository_root)
...
def _validate_family_audit(
    ...
    root_field = "root" if spec.nested_counts else "corpus_root"
    root_value = record.get(root_field)
    if (
        not isinstance(root_value, str)
        or Path(root_value).resolve() != artifact.path.parent
    ):
        raise RuntimeError(f"{spec.name} audit has the wrong corpus root")
    counts = (
    ...
    if spec.name == "tanaka":
        ...
        if identity.get("historical_generation_compatibility_used") is not False:
            raise RuntimeError(
                "Tanaka audit identity does not prove a fresh revision-3 corpus"
            )
    ...
    for index, (raw_chunk, expected) in enumerate(zip(raw_chunks, layout)):
        ...
        if not summary.path.is_relative_to(artifact.path.parent):
            raise ValueError("audit source summary escapes its corpus root")
        ...
        if not manifest.path.is_relative_to(artifact.path.parent) or not (
            ...
            raise ValueError("audit chunk artifact escapes its corpus root")
        source_record = _strict_json(summary.path)
        source_records.append(source_record)
        if (
            source_record.get("schema") != "paper_corpus_quota_summary_v1"
...
def _validate_stokes_legacy(binding_path: Path, legacy_path: Path) -> Artifact:
    ...
    if record.get("schema") != "paper_corpus_cap4_stokes_audit_v1":
...
def authenticate_release(combined_path: Path, audits: AuditPaths) -> ReleaseBinding:
    ...
    specs = (
        ...
        FamilyAuditSpec(
            "tanaka",
            "paper_corpus_tanaka_revision3_completion_audit_v1",
            audits.tanaka,
...
def _renderer_source_proofs(
    ...
    if __package__:
        from scripts import render_paper_corpus_worst_cases as producer
    else:
        import render_paper_corpus_worst_cases as producer
...
def validate_renderer_output(
    ...
    if (
        ...
        raise RuntimeError("renderer population differs from final corpus")
    ...
    *,
    ...
    _require_exact_keys(
        record,
        {
            ...
            "status",
            "diagnostic_only",
            "affects_corpus_acceptance",
            "affects_corpus_release",
            "completion_audit",
            "selection",
    ...
    if (
        ...
        or record.get("affects_corpus_acceptance") is not False
        or record.get("affects_corpus_release") is not False
...
def validate_figure_implementation(value: object) -> dict[str, object]:
    ...
    if __package__:
        from scripts import build_parameterized_corpus_case_figure as producer
    else:
        import build_parameterized_corpus_case_figure as producer
...
def validate_training_output(output: Path, combined: Artifact) -> Mapping[str, object]:
    ...
    sampler = _mapping(
        normalization.get("schema_v2_sampler"), context="training sampler"
    )
    train = _mapping(sampler.get("train_epoch_0"), context="training sampler train")
    validation = _mapping(
        sampler.get("fixed_validation_epoch_0"),
        context="training sampler validation",
    )
    train_families = _mapping(
        train.get("per_family_count"), context="training sampler train families"
    )
    validation_families = _mapping(
        validation.get("per_family_count"),
        context="training sampler validation families",
    )
    if (
        sampler.get("seed") != 0
        or sampler.get("test_rows_in_train_or_validation") != 0
        or train.get("count") != 65_536
        or validation.get("count") != 4_096
        or train.get("one_row_per_case") is not True
        or validation.get("one_row_per_case") is not True
        or set(train_families) != set(FAMILY_ORDER)
        or set(train_families.values()) != {16_384}
        or set(validation_families) != set(FAMILY_ORDER)
        or set(validation_families.values()) != {1_024}
    ):
        raise RuntimeError("training hierarchical sampler contract differs")
    return {
        "output": _artifact(output).record(),
...
def default_stages(
    ...
    return (
        StageSpec(
            ...
            command=(
                python,
                str(ROOT / "scripts/render_paper_corpus_worst_cases.py"),
                "--combined-summary",
                str(combined.path),
                "--require-final-paper-corpus",
                "--output-dir",
                str(renderer_dir),
                "--workers",
        ...
        StageSpec(
            ...
            command=(
                python,
                str(ROOT / "scripts/audit_paper_corpus_training_handoff.py"),
                "--combined-summary",
                str(combined.path),
                "--seed",
                "0",
        ...
        StageSpec(
            ...
            command=(
                python,
                str(ROOT / "scripts/build_parameterized_corpus_case_figure.py"),
                "--combined-summary",
                str(combined.path),
                "--output-stem",
                str(figure_stem),
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    final_root = (
        ROOT / "outputs/paper_corpus_literature_aligned_v1/combined/"
        "c16384_v01024_t01024"
    )
    parser.add_argument(
        "--combined-summary",
        type=Path,
        default=final_root / "paper_corpus_all_splits_c16384.summary.json",
    )
    parser.add_argument(
        "--state-root",
        type=Path,
        default=ROOT / "outputs/paper_corpus_final_postcompletion",
    )
    parser.add_argument("--python", type=Path, default=ROOT / ".venv/bin/python")
    stokes = ROOT / "outputs/paper_corpus_cap4_revision2_20260728"
    bf = (
        ROOT
        / "outputs/paper_corpus_bf_revision4_jonswap_revision3_literature_aligned_v1"
    )
    jonswap = ROOT / "outputs/paper_corpus_jonswap_revision4_relative_band_v1"
    tanaka = ROOT / "outputs/paper_corpus_revision3_literature_aligned_v1"
    parser.add_argument(
        "--stokes-binding",
        type=Path,

"""Rank and render diagnostic tails across completed paper-corpus chunks.

The rankings are descriptive and do not alter corpus acceptance.  Each case
...
from solver.gen_data.clean_tanaka_dataset import (  # noqa: E402
    count_thresholded_sign_changes_vectorized,
)


SUMMARY_PATTERN = re.compile(
    r"^paper_corpus_(?P<family>.+)_(?P<split>train|validation|test)"
    r"\.summary\.json$"
)
...
FINAL_PAPER_CORPUS_SOURCE_COUNT = 26
FINAL_PAPER_CORPUS_SPLIT_ACCEPTED_CASES = {
    "train": 16_384,
    "validation": 1_024,
    "test": 1_024,
}
FINAL_PAPER_CORPUS_ACCEPTED_CASES = 73_728
FINAL_PAPER_CORPUS_RETAINED_ROWS = 7_686_144
DIAGNOSTIC_SCHEMA = "paper_corpus_all_family_diagnostic_tail_v3"
IMPLEMENTATION_SCHEMA = "paper_corpus_renderer_implementation_v1"
IMPLEMENTATION_FILES: Final = (
    ...
    (
        ...
        "thresholded_sign_change_metric_implementation",
    ),
)
...
INTERPRETATION: Final = (
    ...
    "by the frozen corpus construction and trajectory checks. Morphology and "
    "stored-band quadratic-energy diagnostics do not alter corpus acceptance "
    "or constitute release thresholds."
)
PARAMETERS: Final = {
    "final_paper_corpus_contract_required": True,
...
class TrajectoryIndex:
    """Location of one accepted trajectory in a corpus shard."""
...
@dataclass(frozen=True)
class CorpusSource:
    """One complete immutable corpus chunk."""
...
def parse_args() -> argparse.Namespace:
    ...
    sources.add_argument(
        ...
        type=Path,
        help=(
            "Completed corpus chunk root. Repeat for development scans; final "
            "corpus review should use --combined-summary."
        ),
    )
    sources.add_argument(
        "--combined-summary",
    ...
    if args.require_final_paper_corpus and args.combined_summary is None:
        parser.error("--require-final-paper-corpus requires --combined-summary")
    return args
...
def validate_final_paper_corpus_counts(
    *,
    source_count: int,
    ...
    expected_family_split_counts = {
        ...
        for split, split_count in FINAL_PAPER_CORPUS_SPLIT_ACCEPTED_CASES.items()
    }
    observed_family_split_counts = dict(family_split_accepted_cases)
    mismatches: list[str] = []
    if source_count != FINAL_PAPER_CORPUS_SOURCE_COUNT:
        mismatches.append(
            f"sources={source_count}, expected={FINAL_PAPER_CORPUS_SOURCE_COUNT}"
        )
    ...
    if accepted_cases != FINAL_PAPER_CORPUS_ACCEPTED_CASES:
        mismatches.append(
            f"accepted_cases={accepted_cases}, "
            f"expected={FINAL_PAPER_CORPUS_ACCEPTED_CASES}"
        )
    if retained_rows != FINAL_PAPER_CORPUS_RETAINED_ROWS:
        mismatches.append(
            f"retained_rows={retained_rows}, "
            f"expected={FINAL_PAPER_CORPUS_RETAINED_ROWS}"
        )
    if mismatches:
        raise ValueError("final paper-corpus contract failed: " + "; ".join(mismatches))


def validate_final_paper_corpus(
    sources: Sequence[CorpusSource],
    *,
    retained_rows: int,
) -> None:
    ...
    validate_final_paper_corpus_counts(
...
def _summary_identity(path: Path) -> tuple[str, str]:
    ...
    if match is None:
        raise ValueError(f"unrecognized corpus summary name: {path}")
...
def load_source_summary(summary_path: Path) -> CorpusSource:
...
def load_source(root: Path) -> CorpusSource:
    """Load the only completed corpus summary in a development source root."""

    resolved = root.expanduser().resolve(strict=True)
    summaries = tuple(resolved.glob("paper_corpus_*_*.summary.json"))
    if len(summaries) != 1:
        raise RuntimeError(
            f"expected one corpus summary in {resolved}, found {len(summaries)}"
        )
...
def _audit_shard(
    ...
    for start in range(0, row_count, block_rows):
        ...
        thresholded_sign_changes[rows] = count_thresholded_sign_changes_vectorized(
            difference,
...
def _render_to_directory(
    *,
    ...
    if len(cases) != sum(len(source.trajectories) for source in sources):
        raise RuntimeError("whole-corpus audit lost trajectories")
    ...
    if args.require_final_paper_corpus:
        validate_final_paper_corpus(sources, retained_rows=retained_rows)
    if any(
    ...
    figures.extend(
        _plot_cases(
            ...
            overview_frames,
            overview_labels,
            "Largest combined diagnostic rank in each completed corpus family",
    ...
    record = {
        ...
        "parameters": {
            **PARAMETERS,
            "final_paper_corpus_contract_required": bool(
                args.require_final_paper_corpus
            ),
        },

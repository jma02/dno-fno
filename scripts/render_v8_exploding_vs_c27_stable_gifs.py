...
if __name__ == "__main__":
        ...
        if key not in archives:
            ...
            with np.load(path, allow_pickle=False) as loaded_archive:
                archives[key] = RolloutArchive(
                    ...
                    times=np.array(loaded_archive["times"], copy=True),
                    depths=np.array(loaded_archive["depths"], copy=True),
                    truth_eta=np.array(loaded_archive["truth_eta"], copy=True),
                    truth_xi=np.array(loaded_archive["truth_xi"], copy=True),
                    pred_eta=np.array(loaded_archive["pred_eta"], copy=True),
    ...
    for family, simulation_id in simulations:
        ...
        finite_indices = np.flatnonzero(old_finite)
        if finite_indices.size == 0:
            raise ValueError(f"simulation index {old_index} has no finite saved frame")
        last_old = int(finite_indices[-1])
        if np.all(old_finite):
            raise ValueError(
                f"old simulation {family}:{simulation_id} does not explode"
            )
        ...
        old_cumulative_limits = tuple(
            np.maximum.accumulate(_finite_max_per_frame(values))
            for values in old_fields
        )
        old_floors = tuple(
        ...
        for row in range(2):
            for column in range(2):
                ...
                (line,) = axis.plot(
                    x, np.zeros_like(x), color=colors[row], linewidth=1.4
                )
                lines.append(line)
        ...
        def update(frame_index: int) -> list[Any]:
            ...
            for line, label, values in zip(
                lines, amplitude_labels, displayed, strict=True
            ):

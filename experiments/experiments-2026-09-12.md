...
| 2026-09-12 | 20:59 | Equal-row four-family dataset with fresh Stokes states | Equal simulation counts yielded about48% Tanaka,48% BF,4% JONSWAP and0.24% Stokes training exposure. Test the user's simpler equal-family prescription without duplicating static rows or adding a training sampler. | Keep every existing simulation and its split; retain16 of200 saved Tanaka/BF frames, including endpoints, and all16 JONSWAP frames. Generate276480 independent Stokes states, evenly across four parameter groups:138240 per GPU, distinct seeds2026091200/2026091201. Target294912 rows/family,1179648 total. Existing builder remains current-NPZ-only; discarded the proposed existing-array compatibility flag. Logged assembly below reuses the current arrays directly for this experiment. | Source cc21ed6. Eight builder tests and nine additional generator/static-label tests pass; changed-file Ruff/Pyright clean. Runner checks finite fields, exact overall family counts, unchanged existing IDs/splits, no simulation crossing splits and bit-identical failed ICs16471/16624 before publishing. Production validation PENDING. | Launched20:59 on GPU0/GPU1; completion PENDING. | RUNNING: output outputs/paper_dataset_balanced_20260912/arrays; source arrays untouched. No training or resumed reference evaluation. Preserve existing splits, so per-split family proportions are approximately25%, overall exactly25%. Compare future training by optimizer updates, not equal epochs on the smaller dataset. |

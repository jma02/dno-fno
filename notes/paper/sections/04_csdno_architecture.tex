...
Fourier multipliers with pointwise multiplication by powers of the surface
elevation. CS-DNO
...
The Craig--Sulem recurrence motivates two learned ingredients: Fourier
multipliers and spatial multiplication coefficients computed from surface
powers.
Let $\widetilde\eta=\eta/s_\eta$, where $s_\eta>0$ is the fixed training-set
normalization scale. The C27 configuration uses \texttt{cs\_n\_polys=3}, so
the polynomial inputs are explicitly $\widetilde\eta$,
$\widetilde\eta^2$, and $\widetilde\eta^3$. Together with all four enabled
derivative/transform channels, these give the feature stack
\begin{equation}
  E_0(\eta)=
  \left[\widetilde\eta,\widetilde\eta^2,\widetilde\eta^3,
  \partial_x\widetilde\eta,\partial_x^2\widetilde\eta,
  |D|^{1/2}\widetilde\eta,\mathcal H\widetilde\eta\right].
...
A shared pointwise multilayer perceptron with two bias-free dense layers and
GELU activations maps these channels to 320 latent features.
Polynomial channels expose the surface powers used by the recurrence, while
...
Nonlinear mixing allows the learned spatial coefficients to depend on orders
beyond degree three; the separate lift below enforces their second-order
vanishing at the flat surface.

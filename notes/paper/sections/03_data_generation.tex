...
recorded for exact numerical replay.
...
Common modules assign a split and parameter category before numerical work, write
atomic proposal, array-shard, result, and failure records, and build a checked
multi-shard manifest and case-to-row map.  A resumable driver continues until
the accepted-case quota in every parameter category is met.  A rejected attempt is
replaced only in its original category.  The training loader uses the preassigned
splits and draws one stored time from every accepted case, with equal accepted
case counts across the four physical families.  Unexpected construction or
target-evaluation errors propagate and leave the durable proposal available
for replay; they are not silently converted into ordinary rejected samples.
...
The manifest compares the numerical contract and the hashes of its defining
source files only among shards having the same physical family and revision.
A loader-facing view must contain exactly one revision of each included
family.  The final view may therefore combine Stokes revision 2, Tanaka
revision 3, Benjamin--Feir revision 4, and JONSWAP/TMA revision 4, but it
cannot mix two revisions of any one family.
...
while the parameters from which they were constructed are stored in float64.
...
the unfiltered labels in some historical v9 sources.  The generator rejects
changes to $L,N,K,M,p$, or the evaluation precision, and each archive records
a hash of this target configuration and the source files that implement it.
...
\paragraph{Seeded sampling and complete records.}
Let $s$ be the root seed fixed by the data split, let $f$ and $v$ identify the
physical family and generator revision, let $r$ be an integer stream
identifier, and let $i$ be the attempted-case index.  A case uses NumPy PCG64
initialized by a seed sequence with integer entropy $(s,f,v,r,i)$.  The four
...
sequence.  The proposal stores all five seed coordinates and every realized
mode, amplitude, depth, phase, branch, and category.
...
record stores every rejected amplitude and its value of $\mathrm{Ur}_+$ as
well as the accepted draw.  At most 1000 such redraws are permitted.  If this
cap is exhausted, the sampler raises an exception containing the fixed
parameters and every rejected amplitude--$\mathrm{Ur}_+$ pair.  No member of
the attempt is appended as training data.  This event is a declared
out-of-support case, not a fatal batch failure: the production driver stores
the strict record in the proposal, commits a zero-row rejection, and schedules
a replacement in the same category.  Only an explicitly declared deterministic
fatal error writes a terminal failure sidecar.  An ordinary exception
propagates without writing that record, so the attempt can replay from its last
durable state.  Static transactions verify the proposal, archive entries, and
stored configuration hash before commit.
...
complete record as a zero-row out-of-support attempt and scheduled a
...
$\min_\ell\mathcal R_{\alpha,h,x_0}(x_\ell)\geq0$ and records an attempted component
...
The population sampler and the ordered common archive adapter are
implemented.  In particular, the complete sampled parameters are persisted
before the tangent-Hermite constructor is called.  The potential routine
...
a nonpositive or nonfinite computed speed, a malformed diagnostic record, or
an unexpected exception remains an implementation error and stops the run.
...
The common adapter persists the pair, continuous parameters, translation,
and fixed relative phase before constructing the equation-(33) state.  In the
...
Every case proposal record stores both realized phase arrays.  The ordered
adapter persists that record before construction.  It then checks the
...
For every valid construction, the result record includes the discrete peak
wavenumber, the number of spectral cells whose energy is at least half the
largest cell energy, the RMS values of $\eta_0$ and $\xi_0$, the ratio
$4\operatorname{RMS}(\eta_0)/H_s$, the minimum water column, and the relative
error in the exact linear-energy identity below \cref{eq:paper-random-state}.
These are diagnostics of what was constructed, not additional acceptance
rules.  The adapter then connects the result to
the dedicated adjusted trajectory executor, 16-time selection, whole-case
writer, and split-aware view.  An earlier reduced accepted-quota calculation
...
projection to \(P_{128}\), it also records at every saved autonomous frame
...
the initial Hamiltonian and maximum drift, are persisted with the case.
...
complete admissible trajectory.  The individual diagnostics record why this
requirement failed; they are not separately fitted data filters.  An
...
accepted training case once and chooses one of its stored times uniformly.
Because the four families have equal accepted-case counts, every family and
every case have equal epoch weight; the number of stored times does not
determine either weight.  Validation uses one fixed, deterministic stored time
from every accepted validation case.
...
The split-tagged attempt allocator, atomic proposal/shard/result/failure
transaction, multi-shard manifest, and loader are implemented and tested as
family-independent modules.  The loader uses the stored split and chooses one
stored time from every accepted case in each epoch; it requires equal accepted
case counts across families and stops if that condition is absent.  The static
Stokes adapter exercises this transaction at the exact paper target.  Under
...
The Tanaka, Benjamin--Feir, and random-sea adapters enforce sampling, durable
proposal, then construction.  One reduced accepted-quota case from each of
these three families continued through the former paired GL2 validation path
and loader-facing view, and each produced three finite loaded rows.  Those

# Paper-dataset production readiness

Date: 2026-07-25

> **Historical note — superseded production contract.** This note records
> revision-1 and revision-2 measurements and terminology and is retained only
> as historical evidence. The current family-local revisions are Stokes 2,
> corrected Tanaka 3, Benjamin--Feir 4, and JONSWAP/TMA 4. The tables and
> readiness claims below have not been rewritten for those revisions and must
> not be used as a launch or release record. Use the
...
population evidence. No replacement training dataset has yet been generated.
...
3. `tanaka_sampling.py` implements the four cells `main_m1`, `main_m2`,
...
6. `benjamin_feir_sampling.py` makes each of the 66 admissible integer
...
at both steps. That panel is the completed time-step validation, not dataset

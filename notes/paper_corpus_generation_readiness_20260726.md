# Paper-corpus generation readiness
...
The generator-revision-2 104-category trajectory gate is currently running;
bulk generation remains conditional on its completion.
...
checkpoint rather than the final corpus. Here \(C_{\mathrm{tr}}\),
...
The direction-aware Tanaka law advances the global generator revision to 2
and uses the version-2 stored Tanaka parameter record. Revision-1 parameter
smokes, dry runs, and all prior family pilot artifacts are retained as
historical constructor and transaction evidence only. They do not satisfy the
revision-2 run specification and cannot be resumed into a revision-2 run. The
104-category trajectory gate and the entire final corpus use revision 2.
...
\(\epsilon_c\) the carrier steepness, \(\rho\) the
...
symmetric equation-(33) sidebands. The carrier phase is fixed to zero as a
translation gauge.
...
\(\chi_p\delta_s\leq0.15\). Then
\(h=\chi_p/k_p\) and \(H_s=2h\delta_s\). In the finite stratum,
\(k_p\), \(h\), and \(H_s\) are independent uniforms on
\([2,12]\), \([0.1,1.5]\), and \([0.005,0.03]\). In the deep stratum, the
corresponding intervals are \([2,12]\), \([5,25]\), and
\([0.005,0.03]\). Both right- and left-going phase arrays are sampled and
...
## 4. Horizons, retained rows, and exact corpus size

Tanaka and Benjamin--Feir are integrated on \(0\leq t\leq200\), with the
dense result saved every \(0.08\). Each accepted trajectory contributes 200
selected times. Tanaka selection uses relative change in surface-gradient
energy; Benjamin--Feir selection uses fourth-power surface-envelope
excursion. Selection occurs only after acceptance and changes storage, not
quality.
...
the historical v9 corpus: 7,686,144 rather than 7,633,989 rows, a difference
of 52,155 rows (0.68%).
...
- Under bounded run-spec schema 2 and generator revision 1, the dedicated
  ...
  `outputs/paper_corpus_bounded_scheduler_smoke_20260726`. These artifacts
  remain historical transaction evidence.
- The current generator-revision-2 exact Stokes pilot accepted one case in
...
| item | result |
| --- | --- |
| output roots and identities | family runs: `outputs/paper_corpus_exact_smoke_final_20260726`; combined view: `outputs/paper_corpus_exact_smoke_final_view_20260726`; the four run fingerprints are `cbe29081...a189b7`, `45110059...98ebc`, `e6c83d27...a3788`, and `33f8e3c8...f6f4`; the combined dataset-contract fingerprint is `160b1702...2527` |
...
| exact smoke decision | **PASS for the exact target, paired method-validation, transaction, combined-view, integrity, and loader behavior tested under run-spec v1.** It does not test the current one-rollout production executor. That executor is covered by focused software tests; its first exact trajectory-population calculation is the all-category pilot in Section 9. |
...
| --- | --- |
| unified exact Stokes launcher | `outputs/paper_corpus_bounded_scheduler_smoke_20260726/unified_stokes`; attempted/accepted/rows \(1/1/1\); fingerprint `5dbae7f7...a70e4`; complete in \(3.06\) seconds |
| exact four-category Stokes quota | `outputs/paper_corpus_bounded_scheduler_smoke_20260726/stokes_all_cells`; attempted/accepted \(4/4\), exactly one in every category; all fields finite and trainer load passed; fingerprint `5d698085...8e20` |
| reduced real-GL2 trajectory transaction | `outputs/paper_corpus_bounded_scheduler_smoke_20260726/trajectory_reduced`; former paired path; attempted/accepted \(3/3\); strict summary SHA-256 `e17dbb13...8db5` |
...
## 9. Required all-category trajectory GPU pilot

Static Stokes is cleared by the current generator-revision-2 exact
four-category CPU pilot. The remaining gate requests exactly one accepted case from every
trajectory parameter category: 11 Tanaka, 66 Benjamin--Feir, and 27
JONSWAP/TMA cases, for 104 cases in total. It is
deliberately separate from the training corpus and must not be included in the
final combined view. The commands use batch size four so that the random-sea
run exercises variable-horizon batching; the summaries will determine whether
four is an appropriate production batch size. A batch combines distinct
cases in one accelerator call. Each case is still integrated only once, with
...
launched. It should start only after both devices are idle, so its memory and
timing measurements are attributable to this generator.
...
uv run python scripts/run_paper_corpus_quota.py \
  ...
  --stream-id 900 \
  --output-root outputs/paper_corpus_all_category_gpu_pilot_20260727/tanaka \
  --platform gpu \
  --execute
...
uv run python scripts/run_paper_corpus_quota.py \
  ...
  --maximum-attempts-per-accepted-case 4 \
  --stream-id 900 \
  --output-root outputs/paper_corpus_all_category_gpu_pilot_20260727/benjamin_feir \
  --platform gpu \
  --execute
...
uv run python scripts/run_paper_corpus_quota.py \
  ...
  --maximum-attempts-per-accepted-case 4 \
  --stream-id 900 \
  --output-root outputs/paper_corpus_all_category_gpu_pilot_20260727/jonswap_tma \
  --platform gpu \
  --execute
```
...
27 categories, and attempt ceiling four in every nonzero category. The
Tanaka law is now revision 2 with eleven categories, and the production
executor has changed from paired refinement to one rollout. Those old
fingerprints are therefore obsolete. Run the read-only preflight again before
adding `--execute`.

Before adding `--execute`, each command may be run unchanged without that
flag to obtain the read-only preflight. The gate passes only if:
...
builder. The generator records wall time but not peak GPU memory. Run the
following monitor in another terminal for the duration of the pilot, then
stop it with Ctrl-C:

```bash
mkdir -p outputs/paper_corpus_all_category_gpu_pilot_20260727
nvidia-smi \
  --query-gpu=timestamp,index,name,memory.used,memory.total,utilization.gpu,power.draw \
  --format=csv \
  --loop=5 \
  > outputs/paper_corpus_all_category_gpu_pilot_20260727/gpu_telemetry.csv
```

## 10. Exact combined-view commands

Adopt the following output-root layout for the first checkpoint view,
\((C_{\mathrm{tr}},C_{\mathrm{va}},C_{\mathrm{te}})
=(2048,1024,1024)\):

```text
outputs/paper_corpus/train/<family>/chunk_00000_02048
outputs/paper_corpus/validation/<family>/c01024
outputs/paper_corpus/test/<family>/c01024
```

After all twelve checkpoint summaries are complete, run the read-only form of the
following command first. Then repeat it with `--execute`, as shown here, to
write the first all-split manifest:

```bash
uv run python scripts/build_paper_corpus_view.py \
  --chunk-summary outputs/paper_corpus/train/stokes/chunk_00000_02048/paper_corpus_stokes_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/tanaka/chunk_00000_02048/paper_corpus_tanaka_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/benjamin_feir/chunk_00000_02048/paper_corpus_benjamin_feir_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/jonswap_tma/chunk_00000_02048/paper_corpus_jonswap_tma_train.summary.json \
  --chunk-summary outputs/paper_corpus/validation/stokes/c01024/paper_corpus_stokes_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/tanaka/c01024/paper_corpus_tanaka_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/benjamin_feir/c01024/paper_corpus_benjamin_feir_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/jonswap_tma/c01024/paper_corpus_jonswap_tma_validation.summary.json \
  --chunk-summary outputs/paper_corpus/test/stokes/c01024/paper_corpus_stokes_test.summary.json \
  --chunk-summary outputs/paper_corpus/test/tanaka/c01024/paper_corpus_tanaka_test.summary.json \
  --chunk-summary outputs/paper_corpus/test/benjamin_feir/c01024/paper_corpus_benjamin_feir_test.summary.json \
  --chunk-summary outputs/paper_corpus/test/jonswap_tma/c01024/paper_corpus_jonswap_tma_test.summary.json \
  --output-root outputs/paper_corpus/combined/c02048_v01024_t01024 \
  --name paper_corpus_all_splits_c02048 \
  --execute
```

For the final \(C_{\mathrm{tr}}=16384\) view, include all four training chunks
for every family and reuse the fixed validation and test summaries:

```bash
uv run python scripts/build_paper_corpus_view.py \
  --chunk-summary outputs/paper_corpus/train/stokes/chunk_00000_02048/paper_corpus_stokes_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/stokes/chunk_02048_02048/paper_corpus_stokes_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/stokes/chunk_04096_04096/paper_corpus_stokes_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/stokes/chunk_08192_08192/paper_corpus_stokes_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/tanaka/chunk_00000_02048/paper_corpus_tanaka_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/tanaka/chunk_02048_02048/paper_corpus_tanaka_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/tanaka/chunk_04096_04096/paper_corpus_tanaka_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/tanaka/chunk_08192_08192/paper_corpus_tanaka_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/benjamin_feir/chunk_00000_02048/paper_corpus_benjamin_feir_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/benjamin_feir/chunk_02048_02048/paper_corpus_benjamin_feir_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/benjamin_feir/chunk_04096_04096/paper_corpus_benjamin_feir_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/benjamin_feir/chunk_08192_08192/paper_corpus_benjamin_feir_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/jonswap_tma/chunk_00000_02048/paper_corpus_jonswap_tma_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/jonswap_tma/chunk_02048_02048/paper_corpus_jonswap_tma_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/jonswap_tma/chunk_04096_04096/paper_corpus_jonswap_tma_train.summary.json \
  --chunk-summary outputs/paper_corpus/train/jonswap_tma/chunk_08192_08192/paper_corpus_jonswap_tma_train.summary.json \
  --chunk-summary outputs/paper_corpus/validation/stokes/c01024/paper_corpus_stokes_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/tanaka/c01024/paper_corpus_tanaka_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/benjamin_feir/c01024/paper_corpus_benjamin_feir_validation.summary.json \
  --chunk-summary outputs/paper_corpus/validation/jonswap_tma/c01024/paper_corpus_jonswap_tma_validation.summary.json \
  ...
  --chunk-summary outputs/paper_corpus/test/benjamin_feir/c01024/paper_corpus_benjamin_feir_test.summary.json \
  --chunk-summary outputs/paper_corpus/test/jonswap_tma/c01024/paper_corpus_jonswap_tma_test.summary.json \
  --output-root outputs/paper_corpus/combined/c16384_v01024_t01024 \
  --name paper_corpus_all_splits_c16384 \
  --execute
```

The \(C_{\mathrm{tr}}=4096\) and \(8192\) views use the same template,
stopping after the second and third training chunks, respectively. Never
enlarge a completed quota in place.

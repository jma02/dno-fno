...
**v7 blows up ~20% earlier in physical time and with a steeper mid-k growth rate than v5.** This is consistent with the memory that v7_trim regressed relative to v5 — the smaller training corpus produced a surrogate whose 96..256 gxi bias is higher and whose mid-k mode is less damped, so once the a/h≈0.28 Tanaka crest starts feeding the cascade there's less runway.

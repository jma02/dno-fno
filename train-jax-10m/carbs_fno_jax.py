...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run CARBS over the 10M JAX FNO data-parallel trainer")
    parser.add_argument("--dataset", default="tanaka_1_clean.npz")
    parser.add_argument("--trials", type=int, default=20)
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument("--output_root", default="outputs/carbs_jax_10m")
...
def build_carbs(
    ...
    param_spaces = [
        ...
        Param(
            "batch_size",
            LinearSpace(min=64, max=8192, scale=64, is_integer=True, rounding_factor=64),
            search_center=1024,
        ),
        Param(
            "epochs",
            LogSpace(min=5, max=200, is_integer=True),
            search_center=50,
        ),
        Param(
            "modes",
...
def run_trial(
    ...
    run_name = f"carbs_jax_10m_trial_{trial_idx:03d}"
    ...
    child_env.setdefault("NCCL_P2P_LEVEL", "PHB")
